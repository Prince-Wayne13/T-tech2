...
merge, and the real matching key differs by table:
  ...
  - Tables with a unique *_ref column (jobs, invoices, proposals,
    ...
    petty cash) -- the ref IS the stable cross-device identity, because
    ref_generator.py already embeds a per-device fragment in every new
    ref (e.g. "JOB-4F32-0005"), so two devices can never produce the
    same ref for two different records.

  - Capability -- has no _ref column, but `name` is unique=True, so
    name serves the same role.

  - Staff -- has neither a ref nor a unique column. This is the
    genuinely hard case: matched here by (device_id, id), which only
    tells you "this exact row, from this exact device", not "the same
    real person entered twice differently on two machines". Flagged
    explicitly in the report below rather than silently guessed at.
...
# which is a weaker guarantee (see module docstring). Reported with an
# explicit warning in MergePreview rather than treated the same as the
# ref-keyed tables above.
WEAK_KEY_TABLES = ["staff"]
