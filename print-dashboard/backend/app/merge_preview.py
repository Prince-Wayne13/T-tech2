...
from .models import (
    Advance, Capability, Client, ExpenseCategory, ExportJob, Material,
    PricingItem, ProductionMachine, Staff, Vendor,
)
...
SAFE_TO_WRITE_TABLES = {
    ...
    "staff": {
        "model": Staff,
        # Was the weak (device_id, id) key -- see merge_preview.py's
        # WEAK_KEY_TABLES (now empty) and models.py's Staff.staff_ref
        # comment. Once staff_ref existed, this became a normal ref-keyed
        # table like any other; the __device_and_id__ special-casing in
        # _fetch_b_row() below is now unused for staff specifically but
        # left in place (not deleted) in case a future table needs the
        # same weak-key fallback before it gets a real ref column.
    ...
    "pricing_items": {
        ...
        # `code` IS included and copied from B -- it's nullable=False on
        # the model, so leaving it out would crash the INSERT outright.
        # Copying B's code as-is can still collide with a code this
        # device's user already typed for an unrelated item (code is
        # unique=True); that surfaces as a per-row error (see the
        # per-row flush/except handling below) rather than crashing the
        # whole merge or silently renaming a user's chosen code.
        "columns": [
...
# materials) needs to resolve an FK against it.
#
# IMPORTANT, found by actually running this, not by reading the code:
# this list is NOT "only the FK-dependent tables" -- it's the complete
# list of tables the apply loop below processes AT ALL (the loop is
# `for table in APPLY_ORDER`, not `for table in SAFE_TO_WRITE_TABLES`).
# An earlier version of this file left `clients` out of this list on the
# reasoning "it has no incoming FK dependency, so it doesn't need an
# order" -- true but irrelevant, since being absent from this list means
# never applied at all, not applied-in-arbitrary-order. Caught when a
# fake B device's new client silently failed to merge (added: 0) despite
# merge_preview.py correctly reporting it as add_from_b -- the dry-run
# report and what apply_merge actually did had silently diverged, which
# is exactly the failure mode this module's docstring says it's designed
# to prevent. Every key in SAFE_TO_WRITE_TABLES must appear here.
APPLY_ORDER = [
    "production_machines", "capabilities", "vendors", "staff",
    "expense_categories", "advances", "export_jobs", "materials",
    "clients", "pricing_items",
]

# Defensive check for the exact bug class described above: a table
# present in SAFE_TO_WRITE_TABLES but missing from APPLY_ORDER would be
# silently never applied, with no error -- only a quiet "added: 0" that
# looks identical to "nothing needed adding". Failing loudly at import
# time turns that into an immediate, obvious error instead.
                ...
                for table in NOT_YET_SAFE_TABLES:
                    if table in preview_by_table:
                        results.append(TableApplyResult(
                            ...
                            applied=False,
                            skipped_reason=(
                                "References jobs by raw local id (or references "
                                "a table that itself references jobs) which is "
                                "not yet safe to write -- see NOT_YET_SAFE_TABLES "
                                "in merge_apply.py."
                            ),
                        ))
