...
Evidence:
- `backend/app/merge_apply.py:316-318` explicitly lists these as not yet safe:
  - `proposals`
  - `invoice_line_items`
  - `proposal_line_items`
...
- Merge applies invoice headers but does not apply `invoice_line_items` (`backend/app/merge_apply.py:316-318`).
...
- Still open: cross-device merge still does not merge `invoice_line_items`, so full source-line traceability across devices remains unresolved.
...
- But `expense_totals_by_category` adds every expense regardless of paid/unpaid status (`backend/app/services/reports.py:109`).

Impact:
- The same report can say total expenses are paid-only, while category totals include pending/unpaid vendor bills.
...
### 8. Machine revenue and product mix vanish when invoice line items are missing
...
- Merge currently skips `invoice_line_items`.

Impact:
- Revenue dashboard can show invoice revenue, while machine/product reports show zero or undercounted revenue.

Likely fix direction:
- Same root fix as item 3: merge line items, then add reconciliation checks.
...
Impact:
- If `amount` and `quantity * unit_price` ever diverge, proposal totals and converted invoice totals can differ.

Likely fix direction:
- Prefer deriving proposal line amount from `quantity * unit_price`, or enforce/recompute `amount` on every save.
- Add a diagnostic that flags proposal lines where stored amount differs from calculated amount.
...
- If the user changes the selected service before adding it, any previously typed rate can remain attached to the new selected service unless the previous add already cleared it.
...
Likely fix direction:
- Either connect the service picker to real pricing items and set the rate automatically, or deliberately clear `rate` on every new service selection so stale prices cannot carry over.
- Add a visible/manual override price field either way.

### 11. Async machine auto-assignment can attach the wrong machine to a newly added service
...
1. Create a local test proposal with quantity greater than 1, save, reopen edit modal, save again, and compare totals before/after.
2. Create an invoice with line items and payments, run a backup merge dry-run/apply between two sample DBs, then check:
   - invoice header amount
   - invoice line item count
   - payment total
   - machine revenue
   - product mix
3. Add a "money consistency" diagnostic endpoint/script that reports:
