  ...
  return (
    <div className="vendor-item" style={{ position: 'relative', paddingLeft: '14px' }}>
      ...
      <div style={{ display: 'flex', gap: '4px', marginLeft: '8px' }}>
        ...
        <button className="notif-btn" style={{ width: '24px', height: '24px' }} title="Download PDF" onClick={() => onPreview(prop)}><Icon d={D.download} size={11} /></button>
      </div>
    </div>
  );
}
      ...
      <NewProposalModal
        ...
        onSave={(form) => {
          ...
          setPreview({
            ...
            items: form.items,
            total: form.total || form.items.reduce((sum, item) => sum + Number(item.amount || 0), 0),
