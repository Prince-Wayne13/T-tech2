...
function JobRow({ job, onPreview }) {
  ...
  return (
    <div className="vendor-item" style={{ position: 'relative', paddingLeft: '14px' }}>
      ...
      <button className="notif-btn" style={{ width: '24px', height: '24px',color:'black' }} onClick={() => onPreview(job)}><Icon d={D.more} size={12} /></button>
    </div>
  );
}
...
export default function Jobs() {
  ...
  const loadJobs = () => {
    setLoading(true);
    api.jobs('?per_page=100')
      .then(data => setJobs((data.items || []).map(mapJob)))
      .catch(() => setJobs(JOBS_DATA))
  ...
  const stats = [
    { label: 'Active Jobs', value: jobs.filter(j => ['printing', 'queued'].includes(j.status)).length, sub: 'Currently processing', icon: 'printer', color: 'primary' },
    { label: 'Queued', value: jobs.filter(j => j.status === 'queued').length, sub: 'Awaiting start', icon: 'clock', color: 'warning' },
    { label: 'Completed', value: jobs.filter(j => j.status === 'finished').length, sub: 'Ready for pickup', icon: 'check', color: 'teal' },
...
<header style={{ 
    ...
    <h1 style={{ fontSize: '17px', fontWeight: '700', color: '#fff', margin: 0, letterSpacing: '-0.02em', paddingRight: '110px' }}>Jobs</h1>
      ...
      <div className="card" style={{ borderTop: '2px solid var(--primary)' }}>
        <div className="card-header" style={{ marginBottom: '8px' }}><h3 className="card-title">Production Queue</h3><span className="card-sub">{loading ? 'Loading...' : `${filtered.length} job${filtered.length !== 1 ? 's' : ''} found`}</span></div>
        <div className="vendor-items">
          {filtered.map(job => <JobRow key={job.id} job={job} onPreview={setPreview} />)}
        ...
        onSave={async (form) => {
          try {
            const created = await api.createJob({
              ...
              title: form.title || 'New print job',
              priority: form.priority,
              due_date: form.due || null,
              service_category: form.printer || form.specs?.[0],
              notes: [form.notes, form.specs?.join(', ')].filter(Boolean).join('\n'),
            });
            setPreview(mapJob(created));
            loadJobs();
          } catch (error) {
            setPreview({ id: 'Error', title: 'Could not create job', notes: error.message, status: 'queued', progress: 0 });
          } finally {
            setShowEntry(false);
          }
        }}
