...
export default function AuditLog() {
  ...
  const [stats, setStats] = useState([
    { label: 'Total Entries', value: '-', sub: 'All time', icon: 'reports', color: 'primary' },
    { label: "Today's Activity", value: '-', sub: 'Logged actions', icon: 'clock', color: 'warning' },
    { label: 'Critical Events', value: '-', sub: 'Need review', icon: 'alert', color: 'red' },
    { label: 'Active Users', value: '-', sub: 'This month', icon: 'reports', color: 'teal' },
  ]);
      ...
      try {
        ...
        const transformedData = rows.map(entry => {
          ...
          return {
            ...
            time: new Date(entry.created_at || entry.timestamp).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }),
        ...
        const todayCount = transformedData.filter(e => new Date(e.time).toDateString() === today).length;
        setStats(prev => [
          { ...prev[0], value: transformedData.length.toString() },
          { ...prev[1], value: todayCount.toString() },
          { ...prev[2], value: '3' },
          { ...prev[3], value: new Set(transformedData.map(e => e.user)).size.toString() },
        ]);
      } catch (err) {
  ...
  const filtered = auditData.filter(l => {
  ...
  return (
    <main className="main-canvas" style={{ display: 'block' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '18px', paddingBottom: '14px', borderBottom: '1.2px solid rgba(231, 228, 228, 0.4)', position: 'relative' }}>
        ...
        <button onClick={() => downloadPreviewPdf('Audit Log', `<div class="top"><div><h1>T-Tech Audit Log</h1><div>${filtered.length} entries</div></div><div>${new Date().toLocaleDateString()}</div></div><table><thead><tr><th>User</th><th>Action</th><th>Target</th><th>Time</th></tr></thead><tbody>${filtered.map(log => `<tr><td>${log.user}</td><td>${log.action}</td><td>${log.target}</td><td>${log.time}</td></tr>`).join('')}</tbody></table>`)} style={{ background: 'var(--primary)', color: '#fff', border: 'none', borderRadius: '50px', padding: '7px 15px', fontSize: '10px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '5px', cursor: 'pointer', transition: 'all var(--ease)', boxShadow: '0 3px 10px rgba(58,80,107,0.35)' }}
          ...
          <Icon d={D.plus} size={11} /> Download PDF
        </button>
      </header>
      ...
      <div className="card" style={{ borderTop: '2px solid var(--primary)' }}>
        <div className="card-header" style={{ marginBottom: '8px' }}><h3 className="card-title">Activity Feed</h3><span className="card-sub">{loading ? 'Loading...' : filtered.length} log{filtered.length !== 1 ? 's' : ''} found</span></div>
        <div className="vendor-items">
          ...
          {!loading && !error && filtered.map(log => (<LogRow key={log.id} log={log} isExpanded={expandedId === log.id} onToggle={() => setExpandedId(expandedId === log.id ? null : log.id)} />))}
          {!loading && !error && filtered.length === 0 && (<div style={{ textAlign: 'center', padding: '24px', color: 'var(--text-muted)', fontSize: '11px' }}><div style={{ fontSize: '18px', marginBottom: '6px', opacity: 0.4 }}>📋</div>No logs match your filters.</div>)}
        </div>
      </div>
    </main>
  );
