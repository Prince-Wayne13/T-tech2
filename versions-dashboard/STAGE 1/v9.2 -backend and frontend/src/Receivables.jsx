...
function ReceivableRow({ rec, isExpanded, onToggle, onViewDetails, onSendReminder, onRecordPayment }) {
  const statusConfig = {
    ...
    <>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginLeft: '4px' }}>
          <button className="notif-btn" style={{ width: '24px', height: '24px' }} title="View Details" onClick={(e) => { e.stopPropagation(); onViewDetails(rec); }}>
        ...
        <div style={{ 
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end', marginTop: '4px' }}>
              {rec.status !== 'paid' && (
                <button className="notif-btn" style={{ width: '26px', height: '26px' }} title="Send Reminder" onClick={(e) => { e.stopPropagation(); onSendReminder(rec); }}>
                  <Icon d={D.send} size={11} />
                </button>
              )}
              <button className="notif-btn" style={{ width: '26px', height: '26px' }} title="Record Payment" onClick={(e) => { e.stopPropagation(); onRecordPayment(rec); }}>
  ...
  const renderModalContent = () => {
    ...
    switch (modalType) {
      case 'details':
        return (
          <>
            ...
            <div style={{ marginBottom: '12px' }}>
              <div style={{ color: 'var(--text-muted)', marginBottom: '4px' }}>Client:</div>
              <div>{modalData.client}</div>
            </div>
            <div style={{ marginBottom: '12px' }}>
              <div style={{ color: 'var(--text-muted)', marginBottom: '4px' }}>Amount:</div>
              <div style={{ fontWeight: '600', color: 'var(--secondary)' }}>{modalData.amount}</div>
            </div>
            <div style={{ marginBottom: '12px' }}>
      ...
      case 'reminder':
        return (
          <div>
            <div style={{ marginBottom: '12px' }}>Send reminder to <strong>{modalData.client}</strong>?</div>
            <div style={{ padding: '10px', background: 'rgba(33, 150, 243, 0.1)', borderRadius: '6px', fontSize: '11px', color: 'var(--primary)' }}>
              Invoice: {modalData.title}
            </div>
          </div>
        );
      case 'payment':
        return (
          <div>
            <div style={{ marginBottom: '12px' }}>Record payment for <strong>{modalData.title}</strong></div>
            <div style={{ padding: '10px', background: 'rgba(76, 175, 80, 0.1)', borderRadius: '6px', fontSize: '11px', marginBottom: '12px', color: 'var(--teal)' }}>
              Outstanding: <strong>{modalData.amount}</strong>
            </div>
            <div style={{ color: 'var(--text-muted)', fontSize: '11px', marginBottom: '8px' }}>Payment amount:</div>
            <input type="text" placeholder={`Enter amount (e.g., ${modalData.amount})`} style={{
              width: '100%',
              padding: '8px 10px',
  ...
  return (
    <main className="main-canvas" style={{ display: 'block' }}>
      <ActionModal
        ...
        title={
          modalType === 'details' ? 'Receivable Details' :
          modalType === 'reminder' ? 'Send Reminder' :
        ...
        buttons={
          ...
          modalType === 'reminder' ? [
            ...
            { label: 'Send Reminder', variant: 'primary', onClick: () => { setModalOpen(false); setExpandedId(null); } }
          ] :
          modalType === 'payment' ? [
            { label: 'Cancel', onClick: () => setModalOpen(false) },
            { label: 'Record Payment', variant: 'primary', onClick: () => { setModalOpen(false); setExpandedId(null); } }
          ] : []
        }
      >
        {renderModalContent()}
