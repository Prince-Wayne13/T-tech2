...
const normaliseItems = invoice => {
  ...
  return [{ description: invoice?.title || 'Print service', quantity: 1, unit: 'item', unit_price: invoice?.amountValue || invoice?.amount || 0 }];
};
...
function InvoiceDocument({ invoice }) {
  ...
  const total = Number(invoice?.totals?.total ?? invoice?.total_amount ?? invoice?.amountValue ?? subtotal);
