    ...
    Promise.all([api.invoices('?per_page=200'), api.invoiceStats()])
      .then(([invoiceResponse, statsResponse]) => {
        ...
        setInvoices((invoiceResponse.items || []).map(invoice => ({
          ...
          amountValue: invoice.totals?.total || invoice.amount,
  ...
  return (
    <main className="main-canvas" style={{ display: 'block' }}>
      ...
      <NewInvoiceModal
        ...
        onSave={async (form) => {
          if (editRecord?.backendId) {
            await api.updateInvoice(editRecord.backendId, {
              client_name: form.client || editRecord.client,
              title: form.title || editRecord.title,
              due_on: form.due || null,
              notes: form.notes,
              line_items: form.items?.map(item => ({ description: item.desc, quantity: Number(item.qty || 1), unit_price: Number(item.rate || 0), unit: 'item' })),
            });
            setEditRecord(null);
