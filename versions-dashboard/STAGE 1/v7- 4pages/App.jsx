...
const DATASETS = [
  ...
  {
    title: 'Profit',
    a: REVENUE.map((r, i) => r - EXPENSES[i]),
    b: REVENUE.map(() => 0) // 🔥 FIX
  },
  {
    title: 'Cash Flow',
...
useEffect(() => {
  const i = setInterval(() => {
    setSlide(s => (s + 1) % DATASETS.length);
  }, 10000);
  return () => clearInterval(i);
}, []);

  const current = DATASETS[slide];
...
const all = [...dataA, ...dataB];
  ...
  const toX = i => PAD.l + (i / (dataA.length - 1)) * pw;
      ...
      <div className="chart-area fade">
        ...
        >
          <defs>
            ...
            <path 
              d="M20 0 L40 11.54 L40 34.64 L20 46.18 L0 34.64 L0 11.54 Z"
              fill="none"
              stroke="rgba(58,80,107,0.3)"   // neutral yacht tone
              strokeWidth="0.5"
            />
            <path 
              d="M20 0 L40 11.54 L40 34.64 L20 46.18 L0 34.64 L0 11.54 Z"
              fill="none"
              stroke="rgba(58,80,107,0.08)"   // softer layer
              strokeWidth="0.5"
  ...
  <g key={i}>
    ...
    >
      {tick.label}
          ...
          <path
            d={revLine} fill="none"
            stroke="#3A506B" strokeWidth="2.5"
            strokeLinecap="round" strokeLinejoin="round"
            filter="url(#glowGreen)"
          />
          <path
            d={expLine} fill="none"
            stroke="#A06B6B" strokeWidth="2"
            strokeLinecap="round" strokeLinejoin="round"
          ...
          {revPts.map(([x, y], i) => (
            <circle
              ...
              fill={i === revPts.length - 1 ? '#3A506B' : '#0B1E1A'}
              stroke="#3A506B"
          ...
          {expPts.map(([x, y], i) => (
            <circle
              ...
              fill={i === expPts.length - 1 ? '#A06B6B' : '#0B1E1A'}
              stroke="#A06B6B"
          ...
          <text x={lastX} y={lastRevY + 4} fontSize="9" fontWeight="700" fill="#3A506B">$24.2k</text>
          <text x={lastX} y={lastExpY + 4} fontSize="9" fontWeight="700" fill="#A06B6B">$13.8k</text>
        </svg>
      </div>
      ...
      <div className="chart-legend">
        <div className="legend-item">
          <span className="legend-dot" style={{ background: '#3A506B' }} />
          Revenue
        </div>
        <div className="legend-item">
          <span className="legend-dot" style={{ background: '#A06B6B' }} />
          Expenses
        </div>
...
/* ═══════════════════════════════════════
   ROOT COMPONENT — Conditional Page Render
═══════════════════════════════════════ */
