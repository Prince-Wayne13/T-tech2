...
function VendorRow({ vendor, isExpanded, onToggle }) {
  const statusConfig = {
    ...
    <>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginLeft: '4px' }}>
          <button className="notif-btn" style={{ width: '24px', height: '24px' }} title="View Details" onClick={(e) => { e.stopPropagation(); alert(`Details: ${vendor.name}`); }}>
        ...
        <div style={{ 
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end', marginTop: '4px' }}>
              <button className="notif-btn" style={{ width: '26px', height: '26px' }} title="Send Message" onClick={(e) => { e.stopPropagation(); alert(`Message to ${vendor.contact}`); }}>
                <Icon d={D.send} size={11} />
              </button>
              <button className="notif-btn" style={{ width: '26px', height: '26px' }} title="View Orders" onClick={(e) => { e.stopPropagation(); alert(`Orders from ${vendor.name}`); }}>
                <Icon d={D.eye} size={11} />
              </button>
            </div>
          </div>
...
export default function Vendors() {
  ...
  const filtered = VENDORS_DATA.filter(v => {
