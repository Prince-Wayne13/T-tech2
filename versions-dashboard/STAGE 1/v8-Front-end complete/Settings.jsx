...
export default function Settings() {
  ...
  const [pricingItems, setPricingItems] = useState([
    { id: 1, name: 'A4 B&W Printing', value: '500' },
    { id: 2, name: 'A4 Color Printing', value: '1200' },
    { id: 3, name: 'A3 B&W Printing', value: '800' },
    { id: 4, name: 'A3 Color Printing', value: '1800' },
    { id: 5, name: 'Lamination (sheet)', value: '1500' },
    { id: 6, name: 'Binding (copy)', value: '2500' },
    { id: 7, name: 'Delivery Fee', value: '3000' },
  ]);
  ...
  const confirmAdd = () => {
    if (newItem.name && newItem.value) {
      setPricingItems(prev => [...prev, { id: Date.now(), name: newItem.name, value: newItem.value }]);
    ...
    <main className="main-canvas" style={{ display: 'block' }}>
      ...
      <div className="card" style={{ marginBottom: '14px', borderTop: '2px solid var(--secondary)', position: 'relative' }}>
        <div className="card-header" style={{ marginBottom: '14px' }}>
          <h3 className="card-title">Pricing & Fixed Costs</h3>
          ...
          <button 
            ...
            onMouseLeave={e => { e.currentTarget.style.background = 'var(--bg-canvas)'; e.currentTarget.style.color = 'var(--primary)'; }}
          >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '12px' }}>
          {pricingItems.map(item => (
            <div key={item.id} style={{ position: 'relative', background: 'var(--bg-canvas)', padding: '12px', borderRadius: 'var(--r-card)', border: '1px solid var(--border-faint)', display: 'flex', flexDirection: 'column', gap: '4px' }}>
              ...
              <span style={{ fontSize: '10px', color: 'var(--text-muted)', fontWeight: '600', lineHeight: '1.2' }}>{item.name}</span>
              <span style={{ fontSize: '16px', fontWeight: '700', color: 'var(--text-head)', lineHeight: '1.2' }}>MK {Number(item.value).toLocaleString()}</span>
            </div>
          ))}
        </div>
      </div>
