...
function PayableRow({ pay, isExpanded, onToggle, onViewDetails, onSchedulePayment, onViewInvoice }) {
  const statusConfig = {
    ...
    <>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginLeft: '4px' }}>
          <button className="notif-btn" style={{ width: '24px', height: '24px' }} title="View Details" onClick={(e) => { e.stopPropagation(); onViewDetails(pay); }}>
        ...
        <div style={{ 
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end', marginTop: '4px' }}>
              {pay.status !== 'paid' && (
                <button className="notif-btn" style={{ width: '26px', height: '26px' }} title="Schedule Payment" onClick={(e) => { e.stopPropagation(); onSchedulePayment(pay); }}>
                  <Icon d={D.check} size={11} />
                </button>
              )}
              <button className="notif-btn" style={{ width: '26px', height: '26px' }} title="View Invoice" onClick={(e) => { e.stopPropagation(); onViewInvoice(pay); }}>
                <Icon d={D.eye} size={11} />
              </button>
            </div>
          </div>
...
export default function Payables() {
  ...
  const handleSchedulePayment = (pay) => {
    setModalData(pay);
  ...
  const total = filtered.reduce((sum, payable) => sum + Number(String(payable.amount).replace(/[^0-9.-]/g, '')), 0);
  const overdue = filtered.filter(payable => payable.status === 'overdue');
  const due = filtered.filter(payable => payable.status === 'due');
  const stats = [
    { label: 'Total Payable', value: money(total), sub: 'Unpaid bills', icon: 'ap', color: 'warning' },
    { label: 'Overdue Amount', value: money(overdue.reduce((sum, payable) => sum + Number(String(payable.amount).replace(/[^0-9.-]/g, '')), 0)), sub: `${overdue.length} past due`, icon: 'alert', color: 'red' },
    { label: 'Due This Week', value: money(due.reduce((sum, payable) => sum + Number(String(payable.amount).replace(/[^0-9.-]/g, '')), 0)), sub: `${due.length} bills`, icon: 'clock', color: 'secondary' },
    { label: 'Paid This Month', value: money(0), sub: 'No backend payment table', icon: 'check', color: 'teal' },
  ];
  ...
  const renderModalContent = () => {
    ...
    switch (modalType) {
        ...
        return (
          <>
            <div style={{ marginBottom: '12px' }}>
              <div style={{ color: 'var(--text-muted)', marginBottom: '4px' }}>Payable:</div>
              <div style={{ fontWeight: '600' }}>{modalData.title}</div>
            </div>
            <div style={{ marginBottom: '12px' }}>
              <div style={{ color: 'var(--text-muted)', marginBottom: '4px' }}>Vendor:</div>
              <div>{modalData.vendor}</div>
            </div>
            <div style={{ marginBottom: '12px' }}>
              <div style={{ color: 'var(--text-muted)', marginBottom: '4px' }}>Amount:</div>
              <div style={{ fontWeight: '600', color: 'var(--warning)' }}>{modalData.amount}</div>
            </div>
            <div style={{ marginBottom: '12px' }}>
              <div style={{ color: 'var(--text-muted)', marginBottom: '4px' }}>Due Date:</div>
              <div>{modalData.due}</div>
            </div>
            <div>
              <div style={{ color: 'var(--text-muted)', marginBottom: '4px' }}>Notes:</div>
      ...
      case 'schedule':
        return (
          <div>
            <div style={{ marginBottom: '12px' }}>Schedule payment for <strong>{modalData.title}</strong></div>
            <div style={{ padding: '10px', background: 'rgba(255, 152, 0, 0.1)', borderRadius: '6px', fontSize: '11px', marginBottom: '12px', color: 'var(--warning)' }}>
              Amount: <strong>{modalData.amount}</strong>
            </div>
            <div style={{ color: 'var(--text-muted)', fontSize: '11px', marginBottom: '8px' }}>Payment date:</div>
      ...
      case 'invoice':
        return (
          <>
            <div style={{ marginBottom: '12px' }}>
              <div style={{ color: 'var(--text-muted)', marginBottom: '4px' }}>Invoice:</div>
              <div style={{ fontWeight: '600' }}>{modalData.title}</div>
            </div>
            <div style={{ marginBottom: '12px' }}>
              <div style={{ color: 'var(--text-muted)', marginBottom: '4px' }}>Vendor:</div>
            ...
            <div style={{ marginBottom: '12px' }}>
              <div style={{ color: 'var(--text-muted)', marginBottom: '4px' }}>Amount Due:</div>
              <div style={{ fontWeight: '600', color: 'var(--primary)' }}>{modalData.amount}</div>
            </div>
            <div>
              <div style={{ color: 'var(--text-muted)', marginBottom: '4px' }}>Contact:</div>
              <div>{modalData.contact}</div>
            </div>
          </>
        );
      default:
  ...
  return (
    <main className="main-canvas" style={{ display: 'block' }}>
      <ActionModal
        ...
        title={
          modalType === 'details' ? 'Payable Details' :
          modalType === 'schedule' ? 'Schedule Payment' :
          modalType === 'invoice' ? 'Invoice Details' :
          'Action'
        }
        buttons={
          modalType === 'details' ? [
          ...
          modalType === 'schedule' ? [
            ...
            { label: 'Schedule Payment', variant: 'primary', onClick: () => { setModalOpen(false); setExpandedId(null); } }
          ] :
          modalType === 'invoice' ? [
            { label: 'Close', onClick: () => setModalOpen(false) }
          ] : []
