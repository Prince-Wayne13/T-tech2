...
function ReportRow({ rpt, isExpanded, onToggle, onPreview }) {
  const statusConfig = {
    ...
    <>
      <div className="vendor-item" style={{ position: 'relative', paddingLeft: '14px', background: hovered ? 'rgba(255,255,255,0.03)' : 'transparent', transition: 'background var(--ease)', cursor: 'pointer' }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginLeft: '4px' }}>
          <button className="notif-btn" style={{ width: '24px', height: '24px' }} title="Preview" onClick={(event) => { event.stopPropagation(); onPreview(rpt); }}><Icon d={D.eye} size={11} /></button>
    ...
    <main className="main-canvas" style={{ display: 'block' }}>
      ...
      <div className="card" style={{ borderTop: '2px solid var(--primary)' }}>
        ...
        <div className="vendor-items">
          {filtered.map(rpt => (<ReportRow key={rpt.id} rpt={rpt} isExpanded={expandedId === rpt.id} onPreview={setPreview} onToggle={() => setExpandedId(expandedId === rpt.id ? null : rpt.id)} />))}
      ...
      <PrintPreviewModal type="report" title={preview?.name} data={preview ? { title: preview.name, rows: preview.metrics?.machine_revenue || Object.entries(preview.metrics || {}).map(([name, amount]) => ({ name, amount, status: preview.status })), footer: preview.notes } : null} onClose={() => setPreview(null)} />
    </main>
  );
}
