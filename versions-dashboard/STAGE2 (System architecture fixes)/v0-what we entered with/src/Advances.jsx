...
export default function Advances() {
  ...
  useEffect(() => {
    ...
    api.advances('?per_page=200')
      .then(data => {
        if (!active) return;
        setAdvances((data.items || []).map(advance => ({
          id: advance.advance_ref,
          party: advance.recipient,
          title: advance.notes || advance.recipient,
          amount: money(advance.amount),
          date: compactDate(advance.issued_on),
          status: advance.status === 'open' ? 'active' : advance.status,
          remaining: advance.status === 'settled' ? money(0) : money(advance.amount),
  ...
  const total = filtered.reduce((sum, advance) => sum + Number(String(advance.amount).replace(/[^0-9.-]/g, '')), 0);
  const activeTotal = filtered.filter(advance => advance.status === 'active').reduce((sum, advance) => sum + Number(String(advance.amount).replace(/[^0-9.-]/g, '')), 0);
  const settledTotal = filtered.filter(advance => advance.status === 'settled').reduce((sum, advance) => sum + Number(String(advance.amount).replace(/[^0-9.-]/g, '')), 0);
  const stats = [
    { label: 'Total Advances', value: money(total), sub: 'All time', icon: 'advances', color: 'primary' },
    { label: 'Active Balance', value: money(activeTotal), sub: 'Unsettled', icon: 'clock', color: 'warning' },
    { label: 'Settled', value: money(settledTotal), sub: 'Backend records', icon: 'check', color: 'teal' },
    { label: 'Refunded', value: money(0), sub: 'No refunds recorded', icon: 'alert', color: 'secondary' },
  ];
