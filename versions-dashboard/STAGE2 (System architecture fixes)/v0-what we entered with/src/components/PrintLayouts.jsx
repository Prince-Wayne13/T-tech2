...
function layoutToHtml(type, title, data) {
  ...
  return `<div class="top"><div><h1>T-Tech Printing</h1><div>Area 47, Lilongwe</div></div><div><strong>${escape(title || type || 'Document')}</strong><br/>${escape(data.invoice_ref || data.id || '')}</div></div>
    ...
    <div class="amount">${asMoney(data.totals?.total || data.amountValue || data.total || data.amount)}</div>
...
const normaliseItems = data => {
  ...
  return [{ desc: data?.title || 'Print service', qty: 1, rate: Number(data?.rawAmount || data?.amountValue || 0) }];
};

export function InvoicePrintLayout({ data, business = businessDefault }) {
  if (!data) return null;
  const items = normaliseItems(data);
  const subtotal = items.reduce((sum, item) => sum + Number(item.amount ?? Number(item.qty || 1) * Number(item.rate || 0)), 0);
  const vat = Number(data.vat ?? data.tax ?? subtotal * 0.165);
  const total = Number(data.total ?? data.amountValue ?? subtotal + vat);
...
export function ProposalPrintLayout({ data, business = businessDefault }) {
  if (!data) return null;
  const items = data.items?.length ? data.items : [{ desc: data.title || 'Proposal item', amount: data.amountValue || data.total || 0 }];
  const total = Number(data.total || items.reduce((sum, item) => sum + Number(item.amount || 0), 0));
