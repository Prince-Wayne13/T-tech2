...
function ExpenseRow({ exp, isExpanded, onToggle, onViewReceipt, onApprove, onReject, onMarkReimbursed }) {
  const statusConfig = {
    ...
    <>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginLeft: '4px' }}>
          <button className="notif-btn" style={{ width: '24px', height: '24px' }} title="View Receipt" onClick={(e) => { e.stopPropagation(); onViewReceipt(exp); }}>
        ...
        <div style={{ 
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end', marginTop: '4px' }}>
              {exp.status === 'pending' && (
                <>
                  <button className="notif-btn" style={{ width: '26px', height: '26px' }} title="Approve" onClick={(e) => { e.stopPropagation(); onApprove(exp); }}>
                    <Icon d={D.check} size={11} />
                  </button>
                  <button className="notif-btn" style={{ width: '26px', height: '26px' }} title="Reject" onClick={(e) => { e.stopPropagation(); onReject(exp); }}>
                    <Icon d={D.alert} size={11} />
                  </button>
                </>
              )}
              {exp.status === 'approved' && (
                <button className="notif-btn" style={{ width: '26px', height: '26px' }} title="Mark Reimbursed" onClick={(e) => { e.stopPropagation(); onMarkReimbursed(exp); }}>
                  <Icon d={D.check} size={11} />
                </button>
              )}
            </div>
...
export default function Expenses() {
  ...
  const handleMarkReimbursed = (exp) => {
    setModalData(exp);
    setModalType('reimbursed');
    setModalOpen(true);
  };
  ...
  const renderModalContent = () => {
    ...
    switch (modalType) {
        ...
        return (
          <>
            <div style={{ marginBottom: '12px' }}>
              <div style={{ color: 'var(--text-muted)', marginBottom: '4px' }}>Expense:</div>
      ...
      case 'approve':
        return (
          <div>
            <div style={{ marginBottom: '12px' }}>Confirm approval of <strong>{modalData.title}</strong>?</div>
            <div style={{ padding: '10px', background: 'rgba(76, 175, 80, 0.1)', borderRadius: '6px', fontSize: '11px', color: 'var(--teal)' }}>
      ...
      case 'reject':
        return (
          <div>
            <div style={{ marginBottom: '12px' }}>Please confirm rejection of <strong>{modalData.title}</strong>.</div>
            <div style={{ padding: '10px', background: 'rgba(255, 0, 0, 0.1)', borderRadius: '6px', fontSize: '11px', color: 'var(--red)' }}>
              This action will mark the expense as rejected.
            </div>
          </div>
        );
      case 'reimbursed':
        return (
          <div>
            <div style={{ marginBottom: '12px' }}>Mark <strong>{modalData.title}</strong> as reimbursed?</div>
            <div style={{ padding: '10px', background: 'rgba(0, 188, 212, 0.1)', borderRadius: '6px', fontSize: '11px', color: 'var(--teal)' }}>
              Amount: <strong>{modalData.amount}</strong>
            </div>
          </div>
        );
  ...
  return (
    <main className="main-canvas" style={{ display: 'block' }}>
      <ActionModal
        ...
        title={
          modalType === 'receipt' ? 'Receipt Details' :
          modalType === 'approve' ? 'Approve Expense' :
          modalType === 'reject' ? 'Reject Expense' :
          modalType === 'reimbursed' ? 'Mark Reimbursed' :
          'Action'
        }
        buttons={
          modalType === 'receipt' ? [
          ...
          modalType === 'approve' ? [
            ...
            { label: 'Approve', variant: 'primary', onClick: () => { setModalOpen(false); setExpandedId(null); } }
          ] :
          modalType === 'reject' ? [
            { label: 'Cancel', onClick: () => setModalOpen(false) },
            { label: 'Reject', variant: 'primary', onClick: () => { setModalOpen(false); setExpandedId(null); } }
          ] :
          modalType === 'reimbursed' ? [
            { label: 'Cancel', onClick: () => setModalOpen(false) },
            { label: 'Mark Reimbursed', variant: 'primary', onClick: () => { setModalOpen(false); setExpandedId(null); } }
          ] : []
        }
      >
        {renderModalContent()}
      ...
      <AddExpenseModal
        ...
        onSave={(form) => {
          ...
          setModalData({
            id: 'Draft',
            title: form.title || 'New expense draft',
