...
function mapVendor(vendor) {
  return {
    ...
    balanceValue: Number(vendor.balance || 0),
...
function VendorRow({ vendor, onPreview, onEdit }) {
  ...
  const cfg = statusConfig[vendor.status] || statusConfig.current;
...
export default function Vendors() {
  ...
  const loadVendors = () => {
    ...
    api.vendors('?per_page=200')
      .then(data => {
        const items = (data.items || []).map(mapVendor);
        setVendors(items);
  ...
  const totalBalance = filtered.reduce((sum, vendor) => sum + vendor.balanceValue, 0);
  const overdueCount = filtered.filter(vendor => vendor.status === 'overdue').length;
  const activeVendors = filtered.filter(vendor => vendor.status === 'current').length;
  const stats = [
    { label: 'Total Vendors', value: String(filtered.length), sub: 'In directory', icon: D.vendors, color: 'primary' },
    { label: 'Outstanding Balance', value: money(totalBalance), sub: 'Total owed', icon: D.alert, color: 'warning' },
    { label: 'Overdue Accounts', value: String(overdueCount), sub: 'Need attention', icon: D.alert, color: 'red' },
    { label: 'Active Suppliers', value: String(activeVendors), sub: 'Good standing', icon: D.check, color: 'teal' },
  ];
  ...
  return (
    <main className="main-canvas" style={{ display: 'block' }}>
      ...
      <ModuleToolbar filters={categories} filter={filter} setFilter={setFilter} search={search} setSearch={setSearch} placeholder="Search vendor, category, or ID..." />
